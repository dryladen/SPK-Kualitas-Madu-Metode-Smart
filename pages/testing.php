...
function MatriksNormalisasi($koneksi)
{
  ...
  for ($i = 0; $i < count($matriks); $i++) {
    $matriksNormalisasi[$i][0] = $matriks[$i][0]; // nama pegawai
    for ($j = 1; $j < count($matriks[$i]); $j++) {
      // Mencari nilai pembagi
      $pembagi = 0;
      for ($x = 0; $x < count($matriks); $x++) {
        $pembagi += pow($matriks[$x][$j], 2);
      }
      // Mencari nilai normalisasi
      $matriksNormalisasi[$i][$j] = $matriks[$i][$j] / sqrt($pembagi);
    }
  }
...
function Perangkingan($koneksi, $bobot)
{
  ...
  for ($i = 0; $i < count($matriksNormalisasi); $i++) {
    $matriksNormalisasiTerbobot[$i][0] = $matriksNormalisasi[$i][0]; // nama pegawai
    $matriksNormalisasiTerbobot[$i][1] = $matriksNormalisasi[$i][1] * ($bobot[0] / 100); // kehadiran
    $matriksNormalisasiTerbobot[$i][2] = $matriksNormalisasi[$i][2] * ($bobot[1] / 100); // kualitas kerja
    $matriksNormalisasiTerbobot[$i][3] = $matriksNormalisasi[$i][3] * ($bobot[2] / 100); // disiplin
    $matriksNormalisasiTerbobot[$i][4] = $matriksNormalisasi[$i][4] * ($bobot[3] / 100); // kerjasama
    $matriksNormalisasiTerbobot[$i][5] = $matriksNormalisasi[$i][5] * ($bobot[4] / 100); // pengembangan pribadi
    $matriksNormalisasiTerbobot[$i][6] = $matriksNormalisasiTerbobot[$i][1] + $matriksNormalisasiTerbobot[$i][2] + $matriksNormalisasiTerbobot[$i][3] + $matriksNormalisasiTerbobot[$i][4] + + $matriksNormalisasiTerbobot[$i][5]; // nilai y
  }
  // Pengurutan Rangking
  $rangking = $matriksNormalisasiTerbobot;
